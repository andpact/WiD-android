...
//    Log.d("DurationUtil", "getDurationString executed")
    ...
    return when (mode) {
        ...
        1 -> {
            ...
            when {
                hours >= 1 && minutes == 0 -> "${hours}"
                hours >= 1000 -> "${String.format("%.2f", totalHours).substring(0, 6)}" // 시간의 자리 수가 늘어남에 따라 문자열을 다르게 잘라냄.
                hours >= 100 -> "${String.format("%.2f", totalHours).substring(0, 5)}"
                hours >= 10 -> "${String.format("%.2f", totalHours).substring(0, 4)}"
                hours >= 1 -> "${String.format("%.2f", totalHours).substring(0, 3)}"
                minutes >= 1 -> "${minutes}"
                seconds >= 1 -> "${seconds}"
                else -> "0"
            }
